...
import com.portafolio.key_vault_cipher.Vault.utiles.CryptoUtils;
import com.portafolio.key_vault_cipher.Vault.utiles.SecurePreferences;
...
public class SecureDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static synchronized SecureDatabaseHelper getInstance(Context context) {
        if (instance == null) instance = new SecureDatabaseHelper(context);
        return instance;
    }
    ...
    public EncryptedRecord getLastRecord() {
        ...
        net.sqlcipher.Cursor cursor = db.query(TABLE, null, null, null, null, null,
                COL_ID + " DESC", "1");
        if (cursor != null && cursor.moveToFirst()) {
            ...
            String aesKey = cursor.getString(cursor.getColumnIndexOrThrow(COL_AES_KEY));
            record = new EncryptedRecord(data, pubKey, aesKey);
            cursor.close();
        }
        return record;
    }
